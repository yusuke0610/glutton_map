...
type PinsResponse struct {
	...
	// Total ピンの総数（投稿件数。同一ファンの連投も含む）。
	Total int `json:"total"`

	// UniqueFans ip_hash で重複排除した人数。画面の「人」表示にはこちらを使う
	// （total は投稿件数であり連投で水増しされるため）。
...
type PrefectureStat struct {
	// Count その都道府県のユニークファン数（ip_hashで重複排除）。
...
type ServerInterface interface {
	// GetApiPins ヒートマップ用のピン一覧を取得する
	// (GET /api/pins)
	GetApiPins(c *gin.Context)
	// PostApiPins ファンがピンを1件投稿する（認証なし）
	// (POST /api/pins)
	PostApiPins(c *gin.Context)
	// GetPrefectureAt クリック地点の都道府県とそのピン合計件数を返す
